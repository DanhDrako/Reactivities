        ...
        IEmailSender<User> emailSender, IConfiguration config) : BaseApiController
    {
        ...
        public async Task<ActionResult> LoginWithGithub(string code)
        {
            ...
            if (string.IsNullOrEmpty(user.Email))
            {
                ...
                if (!emailResponse.IsSuccessStatusCode) return BadRequest("Failed to fetch user email from GitHub");

                var emails = await emailResponse.Content.ReadFromJsonAsync<List<GitHubEmail>>();
                if (emails == null) return BadRequest("Failed to get user emails");

                var primaryEmail = emails.FirstOrDefault(x => x.Primary && x.Verified);
                if (primaryEmail == null) return BadRequest("Failed to get primary email");
